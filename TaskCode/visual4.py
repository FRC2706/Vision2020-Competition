...
def get_four(width, height, contour):
    ...
    if width or height > 10:
        ...
        cv2.line(negative_mask, leftmost, rightmost, black, thickness, cv2.LINE_AA)
        cv2.line(negative_mask, (middle,0), (middle,height), black, divider, cv2.LINE_AA)
        if booWrite: cv2.imwrite('03-added_lines.jpg', negative_mask)
        ...
        bitwise_bottoms = cv2.bitwise_and(negative_mask,hull_mask)
        ...
        cv2.drawContours(color_bottoms, leftApprox, -1, green, thickness)
        cv2.drawContours(color_bottoms, rightApprox, -1, red, thickness)
        if booWrite: cv2.imwrite('07-approx_points.jpg',color_bottoms)
        ...
        if booWrite: v2.imwrite('08-bottom_points.jpg',color_bottoms)
