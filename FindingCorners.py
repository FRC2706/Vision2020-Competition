...
Folder = input('Press 1 to see OuterTargetDistance images, Press 2 to see OuterTargetHalfScale images: ' )
...
while (True):
    ...
    if initialSortedContours:
        ...
        cv2.drawContours(imgContours, [cnt], -1, blue, 2)
    ...
    cv2.moveWindow('contours and math over green mask',640,0)

    ## loop for user input to close - loop indent 2
    booReqToExit = False # true when user wants to exit

    #shi tomasi corner finding
    gray = cv2.cvtColor(imgImageInput,cv2.COLOR_BGR2GRAY)
    corners = cv2.goodFeaturesToTrack(gray,5,0.01,30)
    ...
    ## wait for user to press key
        ...
        if k == 27:
            ...
            break
        elif k == 91: # user wants previous a
            if i - 1 < 0:
                i = intLastFile
            else:
                i = i - 1
            break
        elif k == 93: # user wants next z
            if i + 1 > intLastFile:
                i = 0
            else:
                i = i + 1
