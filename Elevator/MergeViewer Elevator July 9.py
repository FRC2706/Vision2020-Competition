...
while stayInLoop or cap.isOpened():
    ...
    if moving_since_last_frame == True:
        ...
        if in_moving_state == False:
            ...
            if contour_found == True: #check if the contour and leftmost actually exist
                xArray[0] = leftmost[0]
            else:
                xArray[0] = -1 #dummy
        else:
        ...
        if in_moving_state == True:
            ...
            if contour_found == True:
                print("leftmost[0]=", leftmost[0])
                xArray[num_frames_moving - 1] = leftmost[0]
                last_good_leftmost = leftmost[0]
                print("last_good_leftmost = ", last_good_leftmost)
            else:
                xArray[num_frames_moving - 1] = -1
            ...
            if check_stop_count > check_stop_count_max:
                ...
                total_time_timer = time2 - time1
                print ("total time moving using timer = ", total_time_timer)
                print('tArray[0] is ', tArray[0])
                three_quarter_distance = xArray[0] + 0.75*(last_good_leftmost - xArray[0]) #finding doors 3/4 open
                print("three quarter distance(pixel) = ", three_quarter_distance)
                #july 8 added 
                min_dist = math.inf;
                for i in range(len(xArray)):
                    dist = abs(three_quarter_distance - xArray[i])
