...
#def findBottomCorners(contour, rightmost, leftmost, bottommost)

def findCorners(contour, numCorners): 
    blackImage = zeros([480,640,1],dtype=np.uint8)
    hull = cv2.convexHull(contour)
    # Taking a matrix of size 5 as the kernel 
    #kernel = np.ones((5,5), np.uint8) 

    #image = cv2.drawContours(blackImage, [contour], -1, (255,255,255), cv2.FILLED)
    #image = cv2.drawContours(blackImage, [contour], -1, (255,255,255), cv2.FILLED)
    image = cv2.drawContours(blackImage, [hull], -1, (255,255,255), cv2.FILLED)

    #cv2.fitLine()

    #now use canny edge detection
   ...
   # print("lines: " + str(len(lines)))
    ...
    #img_erosion = cv2.erode(image, kernel, iterations=0) 
    #img_dilation = cv2.dilate(img_erosion, kernel, iterations=0) 
   
    #print('hull contour length = ', len(hull))
    #image = cv2.drawContours(blackImage, [img_erosion], -1, (255,255,255), cv2.FILLED)
    #image = cv2.drawContours(blackImage, [contour], 0, (255,255,255), 1)

    #cv2.dilate(image)

    #gray = cv2.cvtColor(image,cv2.COLOR_BGR2GRAY)
    #corners = cv2.goodFeaturesToTrack(edges,numCorners,0.1,30)
    #corners = cv2.goodFeaturesToTrack(lines,0,0.05,30)
    #corners = cv2.goodFeaturesToTrack(image,numCorners,0.01,40)
    #corners = np.int0(corners)
    #print("corners len: " + str(len(corners)))

    #outer_corners = np.array([leftmost, rightmost, bottommost], dtype="double")
    #sorted_corners = order_points(corners)
    
    #for i in corners:
    #   x,y = i.ravel()
    #   cv2.circle(edges,(x,y),3,(125),-1)
...
def findTape(contours, image, centerX, centerY):
    ...
    if len(contours) >= 1:
        ...
        for cnt in cntsSorted:
            ...
            print("rotation angle" + str(angleofrotation))
            ...
            upperleft_upperRight = np.array([leftmost, rightmost], dtype="double")
            if (bottomIsLeft):
                upperleft_bottomLeft = np.array([leftmost, bottommost], dtype="double")

            else:
                upperright_bottomRight = np.array([rightmost, bottommost], dtype="double")


            rows,cols = image.shape[:2]
            #[vx,vy,x,y] = cv.fitLine(cnt, cv2.DIST_L2,0,0.01,0.01) #errors in VS Code, search online and found fix
            #[vx,vy,x,y] = cv2.fitLine(cnt, cv2.DIST_L2,0,0.01,0.01)
            [vx,vy,x,y] = cv2.fitLine(upperleft_upperRight, cv2.DIST_L2,0,0.01,0.01)
            lefty = int((-x*vy/vx) + y)
            ...
            #find lines that go through top left and right corners
            ls = 0
            for i in range(len(cnt)):
               if (np.all(cnt[i] == leftmost)):
                   ls = i

            #check number of points exists in array (safety check)       
            cntlen = len(cnt)
            #if difference between contour length and position of point is
            #greater than 10, then use 10 points for line of best fit, otherwise use
            #difference
            numpoints = 15
            if (cntlen-(ls+10) < 10):
                numpoints = (cntlen-(ls+10))
            print ("left start: " + str(ls)) 
            left_edge_line = np.array([cnt[ls]])
            for pts in range(numpoints):
                left_edge_line = np.append(left_edge_line,np.array([cnt[ls+pts]]),axis=0)

            print("left edge1: " + str(left_edge_line))
            #left_edge_line = np.array([cnt[ls], cnt[ls+1], cnt[ls+2], cnt[ls+3], cnt[ls+4], cnt[ls+5], cnt[ls+6], cnt[ls+7]], dtype="double")
            print("left edge2: " + str(left_edge_line))

            [vx,vy,x,y] = cv2.fitLine(left_edge_line, cv2.DIST_L2,0,0.01,0.01)
            lefty = int((-x*vy/vx) + y)
            righty = int(((cols-x)*vy/vx)+y)
            cv2.line(image,(cols-1,righty),(0,lefty),(0,255,0),2)

            #find line for rightmost
            ls = 0
            for i in range(len(cnt)):
               if (np.all(cnt[i] == rightmost)):
                   ls = i
            ...
            if ((ls-10) < 0):
                numpoints = ls
            print ("right start: " + str(ls)) 
            right_edge_line = np.array([cnt[ls]])
            for pts in range(numpoints):
                right_edge_line = np.append(right_edge_line,np.array([cnt[ls-pts]]),axis=0)


            print ("right start: " + str(ls)) 
            #left_edge_line = np.array([cnt[ls], cnt[ls-1], cnt[ls-2], cnt[ls-3], cnt[ls-4]], dtype="double")

            [vx,vy,x,y] = cv2.fitLine(right_edge_line, cv2.DIST_L2,0,0.01,0.01)
            lefty = int((-x*vy/vx) + y)
            righty = int(((cols-x)*vy/vx)+y)
            cv2.line(image,(cols-1,righty),(0,lefty),(0,255,0),2)
            ...
            if (bottomIsLeft):
                ...
                if (cntlen-(ls+10) < 10):
                    numpoints = (cntlen-(ls+10))
                print ("bottom start: " + str(ls)) 
                bottom_edge_line = np.array([cnt[ls]])
                for pts in range(numpoints):
                    bottom_edge_line = np.append(bottom_edge_line,np.array([cnt[ls+pts]]),axis=0)

                print("bottom edge: " + str(bottom_edge_line))
                #left_edge_line = np.array([cnt[ls], cnt[ls+1], cnt[ls+2], cnt[ls+3], cnt[ls+4], cnt[ls+5], cnt[ls+6], cnt[ls+7]], dtype="double")
                #print("left edge2: " + str(left_edge_line))

                [vx,vy,x,y] = cv2.fitLine(bottom_edge_line, cv2.DIST_L2,0,0.01,0.01)
                lefty = int((-x*vy/vx) + y)
                righty = int(((cols-x)*vy/vx)+y)
                cv2.line(image,(cols-1,righty),(0,lefty),(0,255,0),2)
            ...
            imageWithCorners = findCorners(cnt,4)
            ...
            if success:
                ...
                cv2.putText(image, "TargetYawToCenter: " + str(angle1), (40, 340), cv2.FONT_HERSHEY_COMPLEX, .6,(255, 255, 255))
