...
def findOuterTarget(frame, mask):
    ...
    if bottommost_is_left == True:
        #which order bottommost and int_point are in
    ...
    camera_matrix = np.array(
                            ...
                            [0, 0, 1]], dtype = "double"
                            ) #replacing h and v focal length from jeremy's code with the calculated matrix from chessboard calibration
    ...
    #distance coefficients calculated from chessboard calibration  
    dist_coeffs = np.array([0.16171387685839064, -0.9962959377099949, -4.1461568670164935e-05, 0.0015151549452291568, 1.2305006106358898], dtype = "double")
    #dist_coeffs = np.zeros((5,1))


    (success, rotation_vector, translation_vector) = cv2.solvePnP(real_world_coordinates, image_points, camera_matrix, dist_coeffs)
    print(dist_coeffs)
    ...
    # Shows the contours overlayed on the original video
    return image, translation_vector, rotation_vector
    ...
    if Driver:
        ...
        if Tape:
            ...
            if PowerCell:
                ...
                processed, translation_vector, rotation_vector = findOuterTarget(frame, threshold)
